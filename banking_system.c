...
void deposit_money() {
    ...
    fclose(file);
    printf("Money could not be deposited as the the Account no. %d was not found in records.", acc_no);
}
    ...
    while (fread(&acc_r, sizeof(acc_r), 1, file)) {
        if (acc_r.acc_no == acc_no) {
            if (acc_r.balance >= money) {
                ...
                fseek(file, -sizeof(acc_r), SEEK_CUR);
                fwrite(&acc_r, sizeof(acc_r), 1, file);
                printf("Successfully withdrawn Rs. %.2f. New balance is Rs. %.2f", money, acc_r.balance);               
            } else {
